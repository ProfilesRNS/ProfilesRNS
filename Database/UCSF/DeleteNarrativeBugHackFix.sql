...
BEGIN
	...
	BEGIN
		-- UCSF hack. If MaxCardinality is set to 1, do not require OldObjectID to be a match
		-- this fixes the delete narrative bug
		SELECT @OldTripleID = TripleID,
			...
			WHERE subject = @SubjectID AND predicate = @PredicateID AND (object = @OldObjectID 
				OR 1 = (SELECT MaxCardinality FROM [Ontology.].[ClassProperty] WHERE MaxCardinality IS NOT NULL AND
				_PropertyNode = @PredicateID))
