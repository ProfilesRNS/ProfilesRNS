...
namespace SemWeb.Stores {
	public abstract class SQLStore : QueryableSource, StaticSource, ModifiableSource, IDisposable {
		...
		private class ResourceKey {
            ...
            public ResourceKey(Int64 id) { ResId = id; }
        ...
        private Int64 GetResourceId(Resource resource, bool create)
        {
        ...
        private Int64 GetResourceIdBuffer(Resource resource, bool create, StringBuilder literalInsertBuffer, StringBuilder entityInsertBuffer, bool insertCombined, ref bool firstLiteralInsert, ref bool firstEntityInsert)
        {
		...
		public void Add(Statement statement) {
            ...
            Int64 subj = GetResourceId(statement.Subject, true);
            Int64 pred = GetResourceId(statement.Predicate, true);
            Int64 objtype = ObjectType(statement.Object);
            Int64 obj = GetResourceId(statement.Object, true);
            Int64 meta = GetResourceId(statement.Meta, true);
		...
		private bool WhereItem(string col, Resource r, System.Text.StringBuilder cmd, bool and) {
			...
			if (r is MultiRes) {
                ...
                Int64 id = GetResourceId(r, false);
		...
		private bool AppendMultiRes(MultiRes r, StringBuilder cmd) {
			...
			for (int i = 0; i < r.items.Length; i++) {
                Int64 id = GetResourceId(r.items[i], false);
				if (id == 0) continue;
        ...
        Entity GetSelectedEntity(Int64 id, string uri, Resource given, bool idSelected, bool uriSelected, Hashtable entMap)
        {
        ...
        Resource GetSelectedResource(Int64 id, Int64 type, string uri, string lv, string ll, string ld, Resource given, bool idSelected, bool uriSelected, Hashtable entMap)
        {
		...
		public void Replace(Statement find, Statement replacement) {
            ...
            Int64 subj = GetResourceId(replacement.Subject, true);
            Int64 pred = GetResourceId(replacement.Predicate, true);
            Int64 objtype = ObjectType(replacement.Object);
            Int64 obj = GetResourceId(replacement.Object, true);
            Int64 meta = GetResourceId(replacement.Meta, true);
