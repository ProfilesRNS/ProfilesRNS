...
namespace Profiles.Profile.Modules.GroupImage
{
    public class GroupCompositePhotoHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        static byte[] silhouetteImage = null;
        static readonly string IMAGE_CACHE_PREFIX = "Image_";

        static GroupCompositePhotoHandler()
        {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(context.Request.QueryString["GroupID"]))
            {

                // get the id for the image
                GroupID = Convert.ToInt32(context.Request.QueryString["GroupID"]);
            }
            if (GroupID > 0)
            {
