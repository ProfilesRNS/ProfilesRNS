    ...
    Init: function(baseURL) {
        if (baseURL) {
            ...
            if (!this._swf_ref) {
                ...
                if (window[this._swf_moviename]) this._swf_ref = window[this._swf_moviename];
                if (document[this._swf_moviename]) this._swf_ref = document[this._swf_moviename];
                if (!this._swf_ref) alert('could not get reference to flash object from DOM');
            }
            try {
    ...
    _flashEventHandler: function(eventName, dataObject) {
        switch (eventName) {
            case "NODE_CTRL_CLICK":
                // view the clicked person's network of co-authors
                window.parent.location = dataObject.uri + network_browser._cfg.profile_network_path;
                break;
            case "NODE_ALT_CLICK":
                // view the clicked person's profile
                window.parent.location = dataObject.uri;
                break;
            case "NODE_CLICK":
