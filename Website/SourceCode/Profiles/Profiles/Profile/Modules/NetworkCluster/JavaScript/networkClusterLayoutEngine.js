 ...
 */
 ...
 ProfilesRNS_ClusterView = {
	...
	Init: function(width,height,targetElement) {
        ...
        ProfilesRNS_ClusterView.data.colors = d3.scale.ordinal().domain([0, 1, 2]).range([d3.rgb("#FF9896"), d3.rgb("#98DF8A"), d3.rgb("#AEC7E8")]);
		// create force engine
...
//		d3.json("data.json", function(error,json) {
			...
			ProfilesRNS_ClusterView.data.d3_svg.selectAll(".link")
				...
				.enter().append("line")
				.attr("class", "link")
				.style("stroke-width", function(d) { return Math.min(Math.pow(d.w,0.75),15); });
			// draw the nodes
			...
			gnodes.append("circle")
				...
				.style("fill", function(d) { return ProfilesRNS_ClusterView.data.colors(d.d); });
	...
	// =========================== PUBLIC FUNCTION ===========================
	dumpEdges: function() {
		console.warn("dumpEdges() not finished");
	},
	// =========================== PUBLIC FUNCTION ===========================
	dumpNodes: function() {
		console.warn("dumpNodes() not finished");
	},
	// =========================== PUBLIC FUNCTION ===========================
	addFilter: function(objType, type, attribute, valueMin, valueMax) {
		console.warn("addFilter() not finished");
	},
	// =========================== PUBLIC FUNCTION ===========================
	removeFilter: function(objType, type, attribute) {
		console.warn("removeFilter() not finished");
	},
	// =========================== PUBLIC FUNCTION ===========================
	...
	_mouse_events: function(eventname, keys, obj, el) {
		...
		switch(externEventName) {
			...
			case "NODE_SHIFT_IN":
				ProfilesRNS_ClusterView.data.d3_svg.selectAll(".link")
					.style('stroke', function(l) { return (obj === l.source || obj === l.target) ? '#F00' : '#999'; })
					.style('stroke-opacity', function(l) { return (obj === l.source || obj === l.target) ? 1 : 0.6; });
				ProfilesRNS_ClusterView.data.d3_svg.selectAll("g.gnode circle")
					.style('stroke', function(d) { return ProfilesRNS_ClusterView._neighboring(d, obj) ? '#F00' : '#000'; })
					.style('stroke-width', function(d) { 
						return ProfilesRNS_ClusterView._neighboring(d, obj) ? '2px' : '0.5px'; 
					});
				break;
			case "NODE_OUT":
			case "EDGE_OUT":
